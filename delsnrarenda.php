...
<?php 
	...
	echo" <h1>Точно убрать из аренды SNR?</h1>
	<form method='POST'>
	...
	$connect = mysqli_connect($host,$name,$password,$db)
		...
		$id = htmlentities($_GET['id']);
		$find_query = "SELECT * FROM SNR_ARENDA WHERE id='$id'";
		...
		$add = "INSERT INTO SNR (name,serial) VALUES ('$name','$serial')";
		$result = mysqli_query($connect,$add) or die ("Ошибка" . mysqli_error($connect));
		
		$del = "DELETE FROM SNR_ARENDA WHERE id = '$id'";
