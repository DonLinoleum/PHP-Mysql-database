...
<?php
	...
	if (isset($_POST['nomer']) && isset($_POST['address']) && isset($_POST['id']) )
	{
		...
		$id = htmlentities($_GET['id']);
		$find_query = "SELECT * FROM SNR WHERE id='$id'";
		...
		$add = "INSERT INTO SNR_ARENDA (name,serial,nomer,address) VALUES ('$name','$serial','$nomer','$address')";
		$result = mysqli_query($connect,$add) or die ("Ошибка" . mysqli_error($connect));
		
		$del = "DELETE FROM SNR WHERE id = '$id'";
